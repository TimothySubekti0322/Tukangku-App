...
const ReviewSummary = () => {
  ...
  const handleSubmit = async () => {
    ...
    const formData = {
      ...
      status: orderServiceCtx.status,
    };
    try {
      const response = await axios.post(`${BASE_URL}/booking/add`, formData, {
        headers: {
      ...
      if (response.status == 201) {
        ...
        Dialog.show({
          ...
          title: "Error",
          textBody:
            "There is missing field in booking Process, please fill it up",
          button: "ok",
        });
      }
